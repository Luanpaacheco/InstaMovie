...
async function voltar() {
  ...
  return (
    ...
    <h1 className="text-4xl font-medium text-white text-left pb-20 pl-10">Seus filmes</h1>
    ...
    <form className="flex flex-row   w-11/12 mt-12 justify-start align-middle ">
      <input 
          ...
          <button type="button" 
                      value="Entrar" 
                      className="cursor-pointer w-fit p-2 mr-44 bg-blue-700 rounded font-medium hover:scale-105 duration-200"
                      onClick={() => teste(outroNickname)}>Pesquisar
          </button>

          <button className=' text-red-700 font-medium bg-red-400 rounded px-6 hover:scale-105 duration-200'
            onClick={() => voltar()}>
            Logout</button>
      </form>
    ...
    <label className="font-medium text-white">title:</label>
          ...
          <input 
            type="number"
            placeholder="De uma nota ao filme"
            className="w-full mb-5 p-2 rounded"
            value={score}
